import { DataFactory, Store, Quad, NamedNode, Writer, Parser } from 'n3';
...
export const buildIntroNp = async (
    ...
    return new Promise((resolve, reject) => {
        ...
        writer.addQuad(twitterNode, namedNode('http://xmlns.com/foaf/0.1/name'), literal(name), assertionGraph);
        writer.addQuad(keyDeclarationNode, namedNode(`${npx}declaredBy`), twitterNode, assertionGraph);
        writer.addQuad(keyDeclarationNode, namedNode(`${npx}hasAlgorithm`), literal('RSA'), assertionGraph);
        writer.addQuad(keyDeclarationNode, namedNode(`${npx}hasPublicKey`), literal(pubKey), assertionGraph);

        writer.addQuad(keyDeclarationNode, namedNode('http://www.w3.org/ns/prov#wasDerivedFrom'), literal(ethAddress), assertionGraph);
        writer.addQuad(keyDeclarationNode, namedNode('https://sense-nets.xyz/hasDerivationPath'), literal('/nanopub/0'), assertionGraph);
        writer.addQuad(keyDeclarationNode, namedNode('https://sense-nets.xyz/hasDerivationWithStandard'), literal('BIP-32'), assertionGraph);

        const derivationProofNode = namedNode(`${BASE_URI}derivationProof`);
        writer.addQuad(derivationProofNode, namedNode(`${npx}hasAlgorithm`), namedNode('https://eips.ethereum.org/EIPS/eip-191'), assertionGraph);
        writer.addQuad(derivationProofNode, namedNode(`${npx}hasPublicKey`), literal(ethAddress), assertionGraph);
        writer.addQuad(derivationProofNode, namedNode(`${npx}hasSignatureTarget`), literal(`This account controls the RSA public key: ${pubKey}`), assertionGraph);
        writer.addQuad(derivationProofNode, namedNode(`${npx}hasSignature`), literal(signature), assertionGraph);

        // Give permission on behalf of user to a key
        if (signDelegation) {
            const signingDelegationNode = namedNode(`${BASE_URI}signingDelegation`);
        writer.addQuad(signingDelegationNode, namedNode(`${npx}declaredAsDelegationBy`), twitterNode, assertionGraph);
        writer.addQuad(signingDelegationNode, namedNode('https://sense-nets.xyz/DelegatedTo'), namedNode('https://sense-nets.xyz/'), assertionGraph);
        writer.addQuad(signingDelegationNode, namedNode('https://sense-nets.xyz/DelegatedBy'), twitterNode, assertionGraph);
        writer.addQuad(signingDelegationNode, namedNode('https://sense-nets.xyz/withKeyDecleration'), namedNode('https://example.org/appKeyDecleration'), assertionGraph);

        }
        
        // Add triples to the provenance graph
        writer.addQuad(namedNode(`${BASE_URI}assertion`), namedNode('prov:wasAttributedTo'), twitterNode, provenanceGraph);
        if (orcidId) {
            const orcidNode = namedNode('https://orcid.org/' + orcidId);
            writer.addQuad(namedNode(`${BASE_URI}assertion`), namedNode('http://www.w3.org/ns/prov#wasAttributedTo'), orcidNode, provenanceGraph);
        }

        // Add triples to the pubinfo graph
        writer.addQuad(namedNode(`${BASE_URI}`), namedNode(`${npx}wasCreatedAt`), namedNode('https://sense-nets.xyz/'), pubinfoGraph);
...
export const buildRetractionNp = async (
    ...
    //const profile = new NpProfile(privateKey, orcidId ? `https://orcid.org/${orcidId}` : undefined, name);

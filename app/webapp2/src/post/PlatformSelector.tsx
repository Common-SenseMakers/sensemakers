...
export const PlatformSelector = (
  ...
  const address =
    connectedUser && connectedUser[PLATFORM.Nanopub]
      ? connectedUser[PLATFORM.Nanopub][0].profile?.ethAddress
      : undefined;

  const hasNano =
    connectedUser !== undefined &&
    connectedUser[PLATFORM.Nanopub] !== undefined;
  ...
  const willNano = canNano && platforms.includes(PLATFORM.Nanopub);
    ...
    <Box
      ...
      {hasNano ? (
        <ToggleButton
          style={{ flexGrow: 1 }}
          enabled={willNano}
          onClick={() => select(PLATFORM.Nanopub)}
          justify="start"
